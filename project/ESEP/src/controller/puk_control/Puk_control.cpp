...
Puk_control::Puk_control() :
    ...
    dispatcher_->addListener(this, LIGHT_BARRIER_RAMP_OPEN_E_ID);
    ...
    dispatcher_->addListener(this, TIMER_CREATE_PUK_OUT_E_ID);
...
puk_list_.erase(std::remove(puk_list_.begin(), puk_list_.end(), p),
        puk_list_.end());
if(puk_list_.empty()){
    timer->stop_timer(entry_timer_id);
}
SEND_REQUEST();
}
...
void Puk_control::TIMER_CREATE_PUK_OUT() {
for (std::vector<Puk_fsm_dummy*>::iterator it = puk_list_.begin();
        it != puk_list_.end(); ++it)
{
    if ((*it)->getState()->timer_create_puk_out())
    {
        return;
    }
}
...
void Puk_control::LIGHT_BARRIER_RAMP_OPEN() {
for (std::vector<Puk_fsm_dummy*>::iterator it = puk_list_.begin();
        it != puk_list_.end(); ++it)
{
    if ((*it)->getState()->light_barrier_ramp_open())
    {
        return;
    }
}
