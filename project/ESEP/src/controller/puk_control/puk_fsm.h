...
private:
	...
	struct Waiting_for_go: public MyState {
		Waiting_for_go() { // Entry
			...
			data->dispatcher->addListener(data->puk_fsm, LIGHT_BARRIER_ENTRY_CLOSE_E_ID);
		}
		virtual void light_barrier_entry_close() {
		...
		// TODO: Update data on system 2|3
			// int systemType = data->puk_control->systemType;
			// if (systemType == 1) {
				// data->puk_control->create_puk(data->pukType);
	...
	struct Height_Sensor_Measure_Register: public MyState {
		Height_Sensor_Measure_Register() {
			// TODO: register for HEIGHT_SENSOR_MEASURE_START
			data->dispatcher->addListener(data->puk_fsm,
		...
		virtual void timer_entry_out() {
			...
			data->dispatcher->remListeners(data->puk_fsm,
					HEIGHT_SENSOR_MEASURE_START_E_ID);
...
public:
	...
	void TIMER_ENTRY_OUT() {
		statePtr->timer_entry_out();
	}
	void TIMER_GROUP_OUT() {
		statePtr->timer_group_out();
	}
	void TIMER_MEASURE_OUT() {
		statePtr->timer_measure_out();
	}
	void TIMER_EXIT_OUT() {
		statePtr->timer_exit_out();
	}
	void identified_register() {
		statePtr->identified_register();
	}
	// TODO: Übergabeparameter mit HSC und Dispatcher absprechen
	void IDENTIFIED_PUK() {
		statePtr->identified_puk();
	}
	void SWITCH_CLOSE() {
		statePtr->switch_close();
	}
	void SEND_OK() {
		statePtr->send_ok();
	}
	void RDY_TAKING_OK() {
		statePtr->rdy_taking_ok();
	}
	// TODO: Errorbehandlung implemetieren
	void ERR_UNDEFINED_PUK_OK() {
	}
	void ERR_LOST_PUK_OK() {
	}
	void ERR_TOO_MANY_PUK_OK() {
	}
